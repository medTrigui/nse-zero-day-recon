...
pluginSAMLSSO.bindTasks = function()
{
	var create_local_user_template_task = $("#create_local_user_template_task", _panel).empty();
	create_local_user_template_task.append("<option value=\"\">-</option>");
	...
	create_local_user_template_task.append(pluginOpts);
	...
	if(availableJobs && availableJobs.length>0)
	{
		...
		create_local_user_template_task.append(jobOpts);
	}
};
...
pluginSAMLSSO.bindData = function(index, pluginID)
{
	...
	if(pluginPrefs)
	{
		...
		if(typeof curPlugin.canonicalization_method == "undefined")
			curPlugin.canonicalization_method = [{text : "INCLUSIVE_WITH_COMMENTS"}];
		if(typeof curPlugin.sign_method  == "undefined")
			curPlugin.sign_method  = [{text : "RSA_SHA1"}];
		if(typeof curPlugin.digest_method  == "undefined")
			curPlugin.digest_method  = [{text : "SHA1"}];
	...
	if($("#create_local_user").is(":checked")){
		...
		$("#create_local_user").closest("div").find(".sub-item").show();
	}
	else{
		$("#create_local_user_template").attr("disabled", "disabled").addClass('ui-state-disabled');
		$("#create_local_user").closest("div").find(".sub-item").hide();
	}
	...
	_plugin.find("input, select, textarea").bind("change", function(){
		...
		else if($(this).is("#create_local_user"))
		{
			if($(this).is(":checked")){
				...
				$(this).closest("div").find(".sub-item").show();
			}
			else{
				$("#create_local_user_template").attr("disabled", "disabled").addClass('ui-state-disabled');
				$(this).closest("div").find(".sub-item").hide();
			}
		}
