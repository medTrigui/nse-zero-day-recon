...
var pluginCrushOAuth = {};
pluginCrushOAuth.localization = {};
/****************************/

// Plugin details
var pluginName = "CrushOAuth";
...
localizations.panels["Plugins"][pluginName] = $.extend(pluginCrushOAuth.localization, localizations.panels["Plugins"][pluginName]);

// Interface methods
pluginCrushOAuth.init = function(pluginID, returnXML){
	...
	pluginCrushOAuth.returnXML = returnXML;
	applyLocalizations(pluginName, localizations.panels["Plugins"]);
	pluginCrushOAuth.bindData(0, pluginID);
...
pluginCrushOAuth.bindData = function(index, pluginID)
{
	...
	index = index || 0;
	var pluginPrefs = [];
	pluginCrushOAuth.showServerList();
	if(pluginID)
	{
	...
	if(pluginPrefs)
	{
        ...
        if(!curPlugin.useLocalDirectory || typeof curPlugin.useLocalDirectory == "undefined" || (curPlugin.useLocalDirectory && !curPlugin.useLocalDirectory[0]))
        {
            curPlugin.useLocalDirectory = [{text: "true"}];
        }
		...
		pluginCrushOAuth.bindPluginDetails(curPlugin);
		if($("#UserHome", _panel).val()){
			_panel.find(".requiredError").hide();
		}
		else{
			_panel.find(".requiredError").show();
		}
		if($("#usernameTemplate", _plugin).val()=="")
		{
			$("#usernameTemplate", _plugin).val("default");
			panelPlugins.itemsChanged(true, pluginCrushOAuth.returnXML, pluginName);
		}
		var service = common;
		if(pluginCrushOAuth.returnXML)
		{
			service = crushFTP;
		}
	...
	pluginCrushOAuth.loadedSubItem = index;
	pluginCrushOAuth.bindEvents(index);
	if($("#authenticationOnly", _plugin).is(":checked"))
	{
		_plugin.find(".non-auth-options").hide();
	}
	else
	{
		_plugin.find(".non-auth-options").show();
	}
	...
	if($("#cognito_auth_enable").is(":checked")){
        $("#amazonCognitoCustomInfo", _panel).show();
    }
    else{
    	$("#amazonCognitoCustomInfo", _panel).hide();
    }
}

pluginCrushOAuth.bindEvents = function()
{
	...
	_plugin.find("input, select, textarea").bind("change", function(){
		...
		if($(this).is("#UserHome")){
			if($(this).val()){
				_plugin.find(".requiredError").hide();
			}
			else{
				_plugin.find(".requiredError").show();
			}
		}
		panelPlugins.itemsChanged(true, pluginCrushOAuth.returnXML, pluginName);
	});
		...
		panelPlugins.pluginMethodCallSaveCallback = function(flag){
			if(flag)
			{
				...
				var obj = {
					...
					method : link.attr("id"),
					pluginName : "CrushOAuth",
					pluginSubItem : pluginCrushOAuth.loadedSubItem == 0 ? "" : _plugin.attr("subPluginName"),
	...
	var ldapRolesList = $("#oauth_roles", _plugin);
	...
	$("a#addNewRole", _plugin).click(function(evt, control){
		...
		if(role.length>0)
		{
			...
			{
				role = $.trim(_temp.shift());
				appendHTML = "<div style='margin:5px 0px;'>Template Username : <br> <input style='margin:5px 0px;width:200px;' id='templateUsernameForRole' class='extraItem' type='text' value='"+ $.trim(_temp.join(":")) +"' /></div>";
			}
		}
	...
	homeDirSubFolders.selectableAdvanced({
		select: function(event, ui) {
			...
			pluginCrushOAuth.showSubFolderPrivs();
			return false;
		},
    ...
    _panel.find("#google_auth_enable, #ms_auth_enable, #azureb2c_auth_enable, #cognito_auth_enable").change(function(){
        if(!$("#google_auth_enable").is(":checked") && !$("#ms_auth_enable").is(":checked") && !$("#azureb2c_auth_enable").is(":checked") && !$("#cognito_auth_enable").is(":checked")){
        	crushFTP.UI.growl("Cant disable all auth options", "", true, 2000);
...
pluginCrushOAuth.showSubFolderPrivs = function(){
	var homeDirSubFolders = $("#homeDirSubFolders", _panel);
	var item = homeDirSubFolders.find("li.ui-selected");
...
pluginCrushOAuth.bindPluginDetails = function(controlData)
{
	...
	var ldapRolesList = $("#oauth_roles", _plugin).empty();
	if(controlData && controlData["oauth_roles"])
	{
		var oauth_roles = controlData["oauth_roles"];
		if(oauth_roles.length>0)
		{
			oauth_roles = oauth_roles[0].text;
			if(oauth_roles)
			{
				oauth_roles = oauth_roles.split("\n");
				for(var i=0;i<oauth_roles.length;i++)
				{
					var curItem = oauth_roles[i];
					if(curItem)
					{
...
pluginCrushOAuth.editRoles = function(){
	var ldapRolesList = $("#oauth_roles", _plugin);
	var roles = "";
...
pluginCrushOAuth.showServerList = function()
{
...
pluginCrushOAuth.saveContent = function(saveByIndex, cloneName, removeByIndex, callback, noGrowl)
{
	removeByIndex = removeByIndex || 0;
	if(pluginPlaceHolder.data("hasChanged") || removeByIndex>0 || (saveByIndex>0 && cloneName) || pluginCrushOAuth.returnXML)
	{
		if(!pluginCrushOAuth.returnXML)
		...
		if(removeByIndex == 0)
		{
			...
			var oauthRoles = $("#oauth_roles", container).find("li");
			if(oauthRoles.length>0)
			{
				xmlString.push("<oauth_roles>");
				oauthRoles.each(function(){
			...
			{
				xmlString.push("<oauth_roles></oauth_roles>");
			}
			var additionalDirs = [];
