...
crushReports.reports.WelcomeFormsSearch = function(data, tpl) {
    ...
    var title = "Welcome Forms Search";
    var desc = "Search items in welcome forms submitted by users.";
        ...
        $(_data).each(function() {
            ...
            metaInfo.each(function(){
                $(this).find("*").each(function(){
                    formInfo[crushFTPTools.xmlEncode($(this).get(0).tagName)] = crushFTPTools.xmlEncode($(this).text());
                });
            });
            transfers.each(function(){
                var curItem = {};
                $(this).find("*").each(function(){
                    curItem[crushFTPTools.xmlEncode($(this).get(0).tagName)] = crushFTPTools.xmlEncode($(this).text());
                });
                transferredItems.push(curItem);
            });
            items.push({
                username : crushFTPTools.xmlEncode(username),
                date : crushReports.getTimestamp(date),
                meta : formInfo,
    ...
    var columns = [{
        ...
        "data": "date",
        render : function(d){
            return crushReports.formatDate(d, true)
        },
        orederData : [3]
    }, {
    ...
    function showFormInfo(data){
        ...
        {
            html += '<div class="row"><div class="col-md-4 text-right"><strong>'+item+' :</strong></div><div class="col-md-8">'+crushFTPTools.decodeXML(formInfo[item]).replace(/\+/g,' ')+'</div></div><hr />'
        }
        var dialog = bootbox.dialog({
            title : 'Form Info : ' + data.username + " (" + crushReports.formatDate(data.date, true) + ")",
            message : html,
            onEscape : true,
            backdrop : true
        });
            ...
            function addType(data, type) {
                ...
                for (var i = 0; i < data.items.length; i++) {
                    ...
                    for(var item in curFile.meta){
                        form.push(crushFTPTools.decodeXML(item) + "=" + crushFTPTools.decodeXML(curFile.meta[item]).replace(/\+/g,' '));
                    }
                    row.push(crushFTPTools.decodeXML(curFile.username));
                    row.push(crushReports.formatDate(curFile.date, true));
                    row.push(form.join("\n"));
