	...
	<xsl:template match="/">
		<html>
			...
			<xsl:if test="$export = 'false'">
				...
				<xsl:variable name="showFormInfo"><xsl:value-of select="results/showFormInfo"/></xsl:variable>
				<head>
					<title>Upload Forms Search</title>
				</head>
				<body bgcolor="#CCCCCC">
					<center><b>Upload Forms Search</b></center>
					<br/>
					...
					<table cellpadding="5">
						...
						<xsl:for-each select="results/uploads/uploads_subitem">
							<tr>
								...
								<td nowrap="nowrap" valign="top"><xsl:if test="$showPaths = 'true'"><xsl:value-of select="path"/></xsl:if><xsl:value-of select="name"/></td>
								<xsl:if test="$showFormInfo = 'true'">
