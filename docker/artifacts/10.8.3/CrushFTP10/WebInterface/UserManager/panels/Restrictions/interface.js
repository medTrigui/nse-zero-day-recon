...
panelRestrictions.bindData = function(userInfo, jsonDeep, panel)
{
	...
	if(userInfo.user)
	{
		...
		if(typeof userInfo.user.socks_site_filter_allow == "undefined")
			userInfo.user.socks_site_filter_allow = "*";
		...
		if(typeof userInfo.user.hint_decrypted_size == "undefined")
			userInfo.user.hint_decrypted_size = "true";
		...
		if(userInfo.user.ip_restrictions)
		{
			...
			userManager.UI.multiOptionControlDataBind(userInfo.user
				...
				, function addNewButton(curItem){
						var type = crushFTP.methods.textEncode(curItem.type);
						var startip = crushFTP.methods.textEncode(curItem.start_ip);
						var stopip = crushFTP.methods.textEncode(curItem.stop_ip);
						var reason = crushFTP.methods.textEncode(curItem.reason);
						var bannedIP = type + startip + "," + stopip;
						return $("<li class='ui-widget-content restriction_"+type+"' bannedIP='"+bannedIP+"'>"+bannedIP+"<div class='ban-info'>" + crushFTP.methods.htmlEncode(reason) + "</div></li>");
					}
			, !panel);
		}
		if(!panel)
		...
		if(userInfo.user.content_restriction)
		{
			...
			for(var i=0;i<items.length;i++)
			{
				...
				if(values.length>=3)
				{
					...
					var newControl = $("<li class='ui-widget-content'>"+crushFTP.methods.htmlEncode(name) + " -> " +crushFTP.methods.htmlEncode(type) + " : " + crushFTP.methods.htmlEncode(extension) + " : " + crushFTP.methods.htmlEncode(path) +"</li>");
...
panelRestrictions.bindEvents = function()
{
	...
	$("a#addIPRestriction", panelRestrictions._panel).click(function(evt, controlData, control) {
        ...
        $("#WIbannedIPDialogBan").dialog({
            ...
            open: function() {
                ...
                $("#WIbannedIPDialogBan #reason").val(reason);
            ...
            buttons: {
                OK: function() {
                    ...
                    reason = crushFTP.methods.htmlEncode($.trim($("#WIbannedIPDialogBan").find("#reason").val()));
                    ...
                    function buildControl() {
						var bannedIP = crushFTP.methods.htmlEncode(chooseOptId + startingipID + "," + stoppingipId);
						var newControl = $("<li class='ui-widget-content restriction_"+chooseOptId+"' bannedIP='"+bannedIP+"'>"+bannedIP+"<div class='ban-info'>" + reason + "</div></li>");
	...
	$("a#pasteBanList", panelRestrictions._panel).click(function(evt) {
        var formbannedIPlist = $("#IPRestrictions", panelRestrictions.panel);
        jPrompt("Paste IP List", "", "CrushFTP IP Insert", function(val, key, extraItemVal, extVals) {
            if (val) {
                var items = val.split("\n");
                var ips = [];
                for (var i = 0; i < items.length; i++) {
                    var curItem = crushFTP.methods.htmlEncode(items[i]);
                    var hasCIDR;
                ...
                if (ips && ips.length > 0) {
                    ...
                    for (var i = 0; i < ips.length; i++) {
                        var curIp = ips[i];
                        curIp.reason = crushFTP.methods.htmlEncode(curIp.reason || "");
                        var bannedIP = crushFTP.methods.htmlEncode(allowDeny + curIp.start + ", " + curIp.end);
                        var newControl = $("<li class='ui-widget-content restriction_"+allowDeny+"' bannedIP='"+bannedIP+"'>"+bannedIP+"<div class='ban-info'>" + curIp.reason + "</div></li>");
						newControl.data("controlData", {
							start_ip:curIp.start,
							type:allowDeny,
							stop_ip:curIp.end,
							reason:curIp.reason || ""
						});
	...
	$("a#addOffset", panelRestrictions._panel).click(function(evt){
		...
		jPrompt("&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Offset : ", offset, "Input", function(value, key, byteValue){
			...
			if(offset != null && byteValue != null)
			{
				...
				{
					var newControl = $("<li class='ui-widget-content'>"+ crushFTP.methods.htmlEncode(offset) + "=" + crushFTP.methods.htmlEncode(byteValue) + "</li>");
...
panelRestrictions.ruleItemSelected = function(list, selected)
{
	...
	if(selected.length>0)
	{
		...
		if(offsetEntries)
		{
			...
			{
				var newControl = $("<li class='ui-widget-content'>"+ crushFTP.methods.htmlEncode(offsetEntries[i]) +"</li>");
			...
			if(panelRestrictions._panel.find("#ipRestrictionsCheck").find("input:checked").length>0)
			{
				...
				$("#IPRestrictions", panelRestrictions._panel).find("li").each(function(){
					...
					if(curItem && curItem.type){
						...
						var reason = curItem.reason || "";
						if(curItem.type && curItem.type !== "undefined")
						{
			...
			if(items.length>0)
			{
				...
				items.each(function(){
					...
					if(controlData)
					{
						...
						var valToSave = crushFTP.methods.htmlEncode(_type + "_" + _name + ";" + path + ";" + extension + ";");
						if(_type == "byte")
						{
...
panelRestrictions.generateXML = function()
{
	...
	if($("#filePublicEncryptionKey").closest('td.settingsArea').prev().find("input[type='checkbox']").is(":checked")){
		$("#filePublicEncryptionKey,#fileDecryptionKey").each(function(){
